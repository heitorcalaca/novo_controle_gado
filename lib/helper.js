...
//matriz por nome
export const getMatrizNome = async (nomeMatriz) => {
  const response = await fetch(
    `${BASE_URL}api/matrizes?nomeMatriz=${nomeMatriz}`
  );
  const json = await response.json();
...
export async function updateMatriz(matrizId, formData) {
  try {
    ...
    if (!response.ok) {
      ...
      return { success: false, error: errorData.message }; // Ou outra estrutura apropriada para o erro
    }
